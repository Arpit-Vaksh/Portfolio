      ...
      {testimonialData.map((person, index) => {
        return(
          <SwiperSlide key={index}>
            <div className='flex flex-col items-center md:flex-row gap-x-2 gap-y-4 h-full px-14 '>
              ...
              <div className='w-full max-w-[300px]  flex flex-col xl:justify-center items-center relative mx-auto xl:mx-0'>
                <div className='flex flex-col justify-center text-center'>
                  ...
                  {/* position */}
                    <div className='text-[10px] mb-5 font-extralight uppercase tracking-widest'>{person.position}</div>
                </div>
              </div>
              {/* message */}
              <div className='flex-1 flex flex-col justify-center -mt-20 md:mt-2 before:w-[1px] xl:before:bg-white/20 xl:before:absolute xl:before:left-0 xl:before:h-[100px] relative xl:pl-20'>
                {/* quote icon */}
                <div className='mb-2'>
                  <FaQuoteLeft className='text-2xl xl:text-6xl text-white/20 mx-auto md:mx-0'/>
                </div>
                  {/* message */}
                  <div className='text-[14px] xl:text-lg text-center md:text-left'>{person.message}</div>
              </div>
            </div>
          </SwiperSlide>
        )
